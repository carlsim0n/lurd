...
const deprecatedRuntimes = require("../../shared/deprecatedRuntimes.js");
...
async function scan(options) {
    ...
    for (const region of scanOptions.regions) {
...
async function storeStacksWithDeprecatedLambdaRuntimes(region, account, filteredStacks, baseDir, stacksFileName, runtimes, delay=500) {
    let content = {
        stacks: [],
        brokenStacks: []
    };
    ...
    for (const stack of filteredStacks) {
        ...
        for (const lambdaResource of stack.LambdaResourceSummaries) {
...
function printResult(content) {
    ...
    const totalStackFunctions = content.stacks.reduce((total, stack) => total + stack.functions.length, 0);
    const totalBrokenStacks = content.brokenStacks.length;
    const totalBrokenStackFunctions = content.brokenStacks.reduce((total, stack) => total + stack.functions.length, 0);
    const output = [{ TotalStacks: totalStacks, TotalStackFunctions: totalStackFunctions, TotalBrokenStacks: totalBrokenStacks, TotalBrokenStackFunctions: totalBrokenStackFunctions}]
