...
async function list(options) {
    ...
    for (const region of listOptions.regions) {
...
function printResult(stacks) {
    const output = [];
    for (const stack of stacks) {
        for (const func of stack.functions) {
            output.push({ region: stack.region, stack: stack.name, function: func.name, runtime: func.runtime});
        }
    }
